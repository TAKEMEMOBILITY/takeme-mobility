...
export default function HeroBookingWrapper({ ctaHref }: { ctaHref: string }) {
  ...
  const confirmRide = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({})) as { checkoutUrl?: string; error?: string; bookingId?: string };
      ...
      <div className="overflow-hidden rounded-3xl border border-[#E5E5EA] bg-white">
        <div className="p-8 text-center">
          ...
          <button
            ...
            className="mt-6 flex w-full items-center justify-center rounded-2xl bg-[#1D1D1F] py-4 text-[16px] font-semibold text-white transition-colors duration-200 hover:bg-[#333]"
          >
            Track your ride
          </button>
        </div>
  ...
  return (
    <div className="overflow-hidden rounded-3xl border border-[#E5E5EA] bg-white">
      ...
      {/* ── Route preview — high contrast, premium ────────────────── */}
      <div className="relative bg-[#1D1D1F] px-6 py-8">
        {/* Subtle grid pattern */}
        <div className="absolute inset-0 opacity-[0.06]">
          {[25,50,75].map(p => <div key={`h${p}`} className="absolute h-[1px] w-full bg-white" style={{ top: `${p}%` }} />)}
          {[25,50,75].map(p => <div key={`v${p}`} className="absolute h-full w-[1px] bg-white" style={{ left: `${p}%` }} />)}
        </div>
        ...
        {hasRoute ? (
          <div className="relative z-10">
            ...
            <div className="flex items-center gap-3 mb-5">
              <div className="flex flex-col items-center gap-1">
                <div className="h-3 w-3 rounded-full border-2 border-[#34C759] bg-[#34C759]" />
                <div className="h-8 w-[2px] bg-gradient-to-b from-[#34C759] to-white/30" />
                <div className="h-3 w-3 rounded-full border-2 border-white bg-white" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="truncate text-[13px] font-medium text-white/70">{pickup?.address}</p>
                <div className="h-6" />
                <p className="truncate text-[13px] font-medium text-white/70">{dropoff?.address}</p>
              </div>
            </div>
            ...
            <div className="flex items-baseline gap-6">
              <div>
                <p className="text-[32px] font-bold tabular-nums text-white">{distanceMiles}</p>
                <p className="text-[12px] font-medium uppercase tracking-wider text-white/40">miles</p>
              </div>
              <div>
                <p className="text-[32px] font-bold tabular-nums text-white">{route!.durationMin}</p>
                <p className="text-[12px] font-medium uppercase tracking-wider text-white/40">min</p>
              </div>
            </div>
          </div>
        ) : routeLoading ? (
          <div className="relative z-10 flex items-center gap-3 py-6">
            ...
            <span className="text-[14px] font-medium text-white/60">Calculating route...</span>
          </div>
        ) : (
          <div className="relative z-10 py-6 text-center">
            <p className="text-[14px] text-white/40">
              {mapsFailed ? 'Enter locations below' : mapsReady ? 'Select pickup & destination' : 'Loading...'}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="p-5">
          ...
          {SEATTLE_TIERS.map(tier => {
            ...
            return (
              <button
                ...
                className={`flex-1 rounded-xl border py-3.5 text-center transition-all duration-150 ${
                  active
              ...
              >
                ...
                <p className={`mt-0.5 text-[12px] tabular-nums font-medium ${active ? 'text-white/60' : 'text-[#86868B]'}`}>
        ...
        <div className="mt-4 flex items-center justify-between rounded-xl bg-[#F5F5F7] px-5 py-4">
          <div>
            ...
            <p className="mt-1 text-[26px] font-bold tabular-nums tracking-tight text-[#1D1D1F]">
              {routeLoading
                ? <span className="inline-block h-6 w-20 animate-pulse rounded bg-[#E5E5EA]" />
                : selectedFare ? `$${selectedFare.total.toFixed(2)}` : '—'}
            </p>
          </div>
          <div className="text-right">
            <p className="text-[11px] font-semibold uppercase tracking-wider text-[#86868B]">Distance</p>
            <p className="mt-1 text-[18px] font-bold tabular-nums text-[#1D1D1F]">
        ...
        <button
          ...
          className={`mt-4 flex w-full items-center justify-center rounded-2xl py-4 text-[16px] font-semibold transition-all duration-200 ${
            ctaDisabled
              ? 'bg-[#E5E5EA] text-[#A1A1A6]'
