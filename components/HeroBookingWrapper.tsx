      ...
      <div className="p-5">
        ...
        <div className="mt-4 -mx-5 relative">
          ...
          <button
            ...
            className="absolute left-1 top-1/2 z-10 -translate-y-1/2 flex h-8 w-8 items-center justify-center rounded-full bg-white/90 shadow-[0_1px_6px_rgba(0,0,0,0.1)] backdrop-blur-sm transition-opacity hover:bg-white"
            aria-label="Scroll left"
          >
            <svg className="h-4 w-4 text-[#1D1D1F]" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" /></svg>
          </button>

          {/* Right arrow */}
          <button
            onClick={() => { carouselRef.current?.scrollBy({ left: 210, behavior: 'smooth' }); }}
            className="absolute right-1 top-1/2 z-10 -translate-y-1/2 flex h-8 w-8 items-center justify-center rounded-full bg-white/90 shadow-[0_1px_6px_rgba(0,0,0,0.1)] backdrop-blur-sm transition-opacity hover:bg-white"
            aria-label="Scroll right"
          >
            <svg className="h-4 w-4 text-[#1D1D1F]" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" /></svg>
          </button>

          {/* Fade edges */}
          <div className="pointer-events-none absolute left-0 top-0 z-[5] h-full w-8 bg-gradient-to-r from-white to-transparent" />
          <div className="pointer-events-none absolute right-0 top-0 z-[5] h-full w-8 bg-gradient-to-l from-white to-transparent" />

          {/* Scrollable row */}
          <div ref={carouselRef} className="flex gap-2 overflow-x-auto px-10 pb-1 scrollbar-none snap-x snap-mandatory">
            {SEATTLE_TIERS.map(tier => {
              ...
              const ICONS: Record<string, { icon: string; color: string }> = {
                electric:          { icon: 'm3.75 13.5 10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75Z', color: '#34C759' },
                comfort_electric:  { icon: 'M9 12.75 11.25 15 15 9.75M21 12c0 1.268-.63 2.39-1.593 3.068a3.745 3.745 0 0 1-1.043 3.296 3.745 3.745 0 0 1-3.296 1.043A3.745 3.745 0 0 1 12 21c-1.268 0-2.39-.63-3.068-1.593a3.746 3.746 0 0 1-3.296-1.043 3.745 3.745 0 0 1-1.043-3.296A3.745 3.745 0 0 1 3 12c0-1.268.63-2.39 1.593-3.068a3.745 3.745 0 0 1 1.043-3.296 3.746 3.746 0 0 1 3.296-1.043A3.746 3.746 0 0 1 12 3c1.268 0 2.39.63 3.068 1.593a3.746 3.746 0 0 1 3.296 1.043 3.746 3.746 0 0 1 1.043 3.296A3.745 3.745 0 0 1 21 12Z', color: '#0071E3' },
              ...
              return (
                <button key={tier.id} onClick={() => setSelectedTier(tier.id)}
                  className={`shrink-0 w-[88px] snap-start rounded-xl border px-2 py-3 text-center transition-all duration-150 ${
                    active
                      ? isWomen ? 'border-[#AF52DE] bg-[#AF52DE] text-white shadow-[0_2px_8px_rgba(175,82,222,0.25)]'
                      : isPetTier ? 'border-[#FF9500] bg-[#FF9500] text-white shadow-[0_2px_8px_rgba(255,149,0,0.25)]'
                  ...
                  <div className="mx-auto flex h-8 w-8 items-center justify-center">
                    {isPetTier ? (
                      ...
                      <svg className={`h-5 w-5 ${active ? 'text-white' : `text-[${ICONS[tier.id].color}]`}`} style={active ? undefined : { color: ICONS[tier.id].color }} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
          ...
          <div className="mt-2 flex justify-center gap-1">
            {SEATTLE_TIERS.map(tier => (
              <div key={tier.id} className={`h-1 rounded-full transition-all duration-200 ${selectedTier === tier.id ? 'w-4 bg-[#1D1D1F]' : 'w-1 bg-[#D2D2D7]'}`} />
            ))}
          </div>
        </div>
