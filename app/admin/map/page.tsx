...
export default function AdminMapPage() {
  ...
  return (
    <div className="relative flex h-full flex-col bg-[#0a0a0f]">
      {/* Top bar */}
      <div className="flex items-center justify-between border-b border-[#1e1e2e] bg-[#0f0f17] px-5 py-3">
        <div className="flex items-center gap-4">
          <h1 className="text-lg font-semibold text-[#e4e4e7]">Live Map</h1>
          <div className="flex items-center gap-2">
            {(['all', 'available', 'busy', 'on_trip', 'offline'] as const).map((s) => (
              ...
              >
                ...
                <span className="ml-1 text-[10px] text-[#52525b]">
                  {driverCounts[s]}
                </span>
              </button>
            ))}
      ...
      <div
        ref={mapRef}
        className="relative flex-1 overflow-hidden bg-[#0d0d15] cursor-grab"
      ...
      >
        ...
        <div className="pointer-events-none absolute inset-0 opacity-10">
          <div className="absolute left-1/2 top-0 bottom-0 w-px bg-[#3f3f5e]" />
          <div className="absolute top-1/2 left-0 right-0 h-px bg-[#3f3f5e]" />
        </div>
        ...
        {rides.map((ride) => {
          ...
          return (
            <div key={`ride-${ride.id}`}>
              ...
              >
                <div className="flex h-5 w-5 items-center justify-center rounded-full border-2 border-emerald-400 bg-emerald-400/30">
        ...
        <div className="absolute right-4 top-4 z-40 flex flex-col gap-1">
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#13131b] border border-[#1e1e2e] text-[#e4e4e7] hover:bg-[#1e1e2e] transition-colors"
          >
            +
          </button>
          <button
        ...
        {/* Coordinate display */}
        <div className="absolute left-4 bottom-4 z-40 rounded-lg bg-[#13131b]/90 border border-[#1e1e2e] px-3 py-1.5 text-[10px] font-mono text-[#52525b]">
        ...
        {selectedDriver && (
          <div
            className="absolute z-50 w-64 rounded-xl bg-[#13131b] border border-[#1e1e2e] p-4 shadow-2xl -translate-x-1/2"
            style={{
          ...
          >
            ...
            <div className="space-y-1 text-xs text-[#a1a1aa]">
              <div className="flex justify-between">
                <span>Status</span>
                <span className="font-medium capitalize text-[#e4e4e7]">
              ...
              <div className="flex justify-between">
                <span>Trips</span>
                <span className="text-[#e4e4e7]">{selectedDriver.total_trips}</span>
              </div>
              ...
              {selectedDriver.speed_kmh != null && selectedDriver.speed_kmh > 0 && (
                <div className="flex justify-between">
                  <span>Speed</span>
                  <span className="text-[#e4e4e7]">{selectedDriver.speed_kmh} km/h</span>
                </div>
              )}
            </div>
          </div>
        ...
        {selectedRide && (
          <div
            className="absolute z-50 w-72 rounded-xl bg-[#13131b] border border-[#1e1e2e] p-4 shadow-2xl"
            style={{
          ...
          >
            ...
            <div className="space-y-1.5 text-xs text-[#a1a1aa]">
              ...
              {selectedRide.driver_name && (
                <div className="flex justify-between">
                  <span>Driver</span>
                  <span className="text-[#e4e4e7]">{selectedRide.driver_name}</span>
                </div>
              )}
              {selectedRide.rider_name && (
                <div className="flex justify-between">
                  <span>Rider</span>
                  <span className="text-[#e4e4e7]">{selectedRide.rider_name}</span>
                </div>
              )}
            </div>
          </div>
