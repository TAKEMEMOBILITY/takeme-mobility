...
export default function AdminFleetPage() {
  ...
  if (loading) return <div className="flex min-h-screen items-center justify-center text-[#86868b]">Loading fleet data...</div>;

  const m = data?.metrics;

  return (
    <div className="min-h-screen bg-[#FFFFFF] p-6 lg:p-8 text-[#1d1d1f]">
      <h1 className="text-xl font-semibold text-[#1d1d1f]">Fleet Management</h1>
      <p className="mt-1 text-[13px] text-[#86868b]">EV owner onboarding, vehicle approval, bookings</p>
      ...
      <div className="mt-6 grid grid-cols-2 gap-4 lg:grid-cols-5">
        {[
          ...
          <div key={s.label} className="rounded-xl border border-[#d2d2d7] bg-[#f5f5f7] p-4">
            <p className="text-[11px] font-semibold uppercase tracking-wider text-[#86868b]">{s.label}</p>
            <p className="mt-2 text-2xl font-bold text-[#1d1d1f]">{s.value}</p>
          </div>
        ))}
      </div>
      ...
      {/* Pending Owners */}
      <div className="mt-8">
        <h2 className="text-[15px] font-semibold text-[#1d1d1f]">Pending owner approvals</h2>
        <div className="mt-3 space-y-2">
          {(data?.pendingOwners ?? []).map(o => (
            <div key={o.id} className="flex items-center justify-between rounded-xl border border-[#d2d2d7] bg-[#f5f5f7] p-4">
              <div>
                <p className="text-[14px] font-medium text-[#1d1d1f]">{o.email}</p>
                <p className="text-[12px] text-[#86868b]">{o.status.replace(/_/g, ' ')} &middot; {ago(o.created_at)}</p>
              </div>
          ...
          {(data?.pendingOwners ?? []).length === 0 && <p className="text-[13px] text-[#86868b]">No pending approvals.</p>}
        </div>
      </div>

      {/* Pending Vehicles */}
      <div className="mt-8">
        <h2 className="text-[15px] font-semibold text-[#1d1d1f]">Pending vehicle reviews</h2>
        <div className="mt-3 space-y-2">
          {(data?.pendingVehicles ?? []).map(v => (
            <div key={v.id} className="flex items-center justify-between rounded-xl border border-[#d2d2d7] bg-[#f5f5f7] p-4">
              <div>
                <p className="text-[14px] font-medium text-[#1d1d1f]">{v.year} {v.make} {v.model}</p>
                <p className="text-[12px] text-[#86868b]">{ago(v.created_at)}</p>
              </div>
              <div className="flex gap-2">
          ...
          {(data?.pendingVehicles ?? []).length === 0 && <p className="text-[13px] text-[#86868b]">No pending reviews.</p>}
        </div>
      </div>
    </div>
  );
